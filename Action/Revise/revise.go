...
func Revising(t sdkInit.Application, args []string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf(">>failed to revise: %v", err)
	}
	return string(response.Payload), nil
}

func main() {
	// jeweler string, paperNumber string, reviseDateTime string, financingAmount int
	args := []string{"Revise", "jeweler01", "002", "2022/01/01", "1000"}
